...
class Agenticos < Formula
  ...
  url "https://github.com/madlouse/homebrew-agenticos/releases/download/v0.6.0/agenticos-mcp-0.6.0.tgz"
  version "0.6.0"
  sha256 "df84c095cd7f03d9ccf8b2ebf829076b06db18de3844af2a348ce7da7b1d24f6"
  license "MIT"

  depends_on "node"

  def install
    empty_cache = buildpath/"agenticos-empty-npm-cache"
    rm_rf empty_cache
    empty_cache.mkpath
    # The release contract requires proving the bundled closure without Homebrew's shared npm cache.
    begin
      system "npm", "install", *std_npm_args, "--offline", "--cache=#{empty_cache}",
             "--no-audit", "--no-fund"
    ensure
      rm_rf empty_cache
    end
  ...
  def post_install
    ...
    ohai "First-run mode installs AgenticOS activation Skills for Codex, Claude Code, " \
         "Cursor, Gemini CLI, and Hermes Agent when selected."
    ...
    ohai "Then run: agenticos-bootstrap --workspace \"#{var}/agenticos\" --all " \
         "--install-skills --auto-configure-hooks --verify"
  ...
  def caveats
    <<~EOS
      ...
      6. Product policy: Homebrew installation itself is reminder-only.
         It does not mutate user agent configs by default. The shipped
         agenticos-bootstrap command performs only the explicit --apply/--first-run
         operations requested by the user; silent config mutation is out of scope.
    EOS
  end

  test do
    package_root = libexec/"lib/node_modules/agenticos-mcp"
    metadata = JSON.parse((package_root/"package.json").read)
    bins = metadata.fetch("bin")
    assert bins.key?("agenticos-mcp"), "package CLI inventory omits agenticos-mcp"
    bins.each do |name, target|
      installed = bin/name
      assert_predicate installed, :executable?
      assert_equal (package_root/target).realpath, installed.realpath
      assert installed.realpath.to_s.start_with?("#{prefix.realpath}/")
    end
